...
处理方式阶段和试读阶段的反馈合计最多 5 次。用户最终采用试读对应的处理方式后，session
...
`strategy_draft_version`、节点位置和 block range 的内容生成任务，并在任务完成后恢复 session。
...
- 已读节点保留旧版本
- 用于调整预览的节点只有在用户查看完整候选效果并明确采用后，才切换为预览结果
- 其他当前或已读节点不被后台突然替换
...
工具只创建或修订 pending proposal，不生效。同一 user-book 同时只允许一个活动 proposal。

用户选择“看看调整后的效果”后，业务系统基于 proposal 创建候选策略草稿，并使用正式内容
生成器对一个完整的可裁读正文节点生成预览：当前位置有裁读资格时使用当前节点，否则使用
距离最近的可裁读节点并明确告知用户来源。预览写入独立 namespace，不能覆盖正式内容。

用户查看完整预览并明确采用后，业务系统才创建新的正式策略版本，把预览结果提升为该节点
的新正式结果，并安排未读节点按新策略重新生成；其他已读节点保留旧版本。用户拒绝、继续
修改或预览失败时，当前正式策略和正式节点内容保持不变。

### 8.3 禁止事项

- 不修改原文或规范化书籍
- 不直接应用处理方式变更
- 不把文字建议确认或预览生成完成当作策略正式生效
- 不访问其他用户数据
- 不为了回答一个问题加载整本书
- 不在没有证据时更新长期画像
...
- [ ] 更新长期画像不需要确认但必须有证据
- [ ] 本书处理方式只能创建待确认建议
- [ ] 建议必须生成完整可裁读正文节点预览，用户查看并采用后才能生效
- [ ] 预览失败或拒绝不影响当前正式策略和阅读内容
