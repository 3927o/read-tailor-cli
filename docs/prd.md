...
这一步在 `book.normalized.html` 的基础上进行：

1. 提取出结构化的注释信息，生成 `notes.json`
2. 删除 `book.normalized.html` 中的注释正文内容
3. 仅保留正文中的注释引用 `a[data-role="noteref"]`
4. 为后续按策略重新注入、改写、折叠或重排注释做准备

这一步是基于标准化 HTML 结构执行的**固定内置处理步骤**，不为单本书额外生成注释提取脚本。不同书籍的注释结构差异应尽量在 Step 2 的规范化阶段被收敛；Step 3 只负责按统一规则提取 note、重建引用关系并剥离注释正文。
...
- 脚注、尾注、章末注统一抽象为 note；若能识别类型，可额外记录 `kind`
- 一个注释若被多个位置引用，`notes.json` 中应保留 `refs` 数组，而不是只保留单个引用
- 若原始注释缺少可靠 id，应基于文档顺序与所在章节位置重新生成稳定 id
- 若注释正文中还包含二级注释、交叉引用或复杂块结构，应优先完整保留原始 HTML 片段
...
- 每条 note 最少包含：
  ...
  - `kind`：注释类型；无法判断时可为 `unknown` 或留空
  - `chapter_id`：注释归属章节；无法判断时可留空
  - `order`：该 note 在全书中的顺序
  - `source`：原始锚点信息，至少包括 `original_note_id` 与 `original_href_target`
  - `refs`：正文中的引用位置数组；一个 note 可对应多个引用
  - `content.html`：原始注释正文的 HTML 片段，优先完整保留
